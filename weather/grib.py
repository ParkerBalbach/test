...
class Raster():
    ...
    def get_min_loc(self):
            ...
            self.download_with_retry(('ds.temp.bin','min','https://tgftp.nws.noaa.gov/SL.us008001/ST.opnl/DF.gr2/DC.ndfd/AR.pacnwest/VP.001-003/ds.temp.bin'))
            pos_north = self.pos_north
            pos_west = self.pos_west
            # grib needs coordinates positive
            pos_west = -pos_west
            file_path = current_dir+'\\ds.temp.bin'
            output_raster = current_dir+"\\temp.grb"

            # Set up projection, open gribfile, 
            dst_srs = osr.SpatialReference()
            dst_srs.ImportFromEPSG(4326)
            dst_wkt = dst_srs.ExportToWkt()
            # gf = 'ds.temp.bin'
            gf_obj = gdal.Open(file_path, 0)
            rc = gf_obj.RasterCount
            ds_out = gdal.AutoCreateWarpedVRT(gf_obj, None, dst_wkt)


            grib_type = 'max'
            # print(os.getcwd()+'\\ds.temp.bin') 
            temp_list= []
            ...
            # The top left of the grid array at (0,0) is the max latitude and min longitude
            # We need to get the point's coordinate distance from the origin to get the position in the grid. 
            x_distance_from_origin = abs(positive_minx-pos_west) 
            y_distance_from_origin = abs(maxy-pos_north)
            

            # Dividing coordinate distance by size of each pixel gets us the number of grid points from the origin.
            # Now we have the location on the grid and can pull it from the array.
            col = math.floor(x_distance_from_origin/x_pixel_size)
            row = math.floor(y_distance_from_origin/y_pixel_size)

            print('row (y): {}, col(x): {}'.format(row, col))
...
# Create the raster object with coordinates
raster = Raster(43.6,-116.3, True)
# Print each temp in the bands
raster.get_min_loc()
